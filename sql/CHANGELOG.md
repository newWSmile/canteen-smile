...
| `IAM_DDL_0010` | IAM | DDL | `iam/ddl/IAM_DDL_0010__add_generic_async_audit_metadata.sql` | `IAM_DDL_0009` | `EXECUTED` | 用户确认已在 LOCAL PostgreSQL 手工执行；为注解驱动异步审计增加事件幂等 ID、端和任意层级分类快照、失败码与耗时字段 |
| `IAM_DDL_0011` | IAM | DDL | `iam/ddl/IAM_DDL_0011__add_audit_client_ip.sql` | `IAM_DDL_0010` | `EXECUTED` | 用户确认已在 LOCAL PostgreSQL 手工执行；为所有 IAM 管理操作审计增加由网关确认的完整客户端 IP 快照，供受控安全追查 |
...
| `AUTH_DDL_0014` | AUTH | DDL | `auth/ddl/AUTH_DDL_0014__store_full_client_ip.sql` | `AUTH_DDL_0013` | `EXECUTED` | 用户确认已在 LOCAL PostgreSQL 手工执行；登录设备与认证审计现保存完整客户端地址，历史脱敏值不回填 |
