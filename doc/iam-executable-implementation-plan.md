...
实施记录（2026-08-12）：Gateway、Auth、IAM 已形成独立部署和独立数据边界，平台管理端、租户管理端、租户业务端三个 Vue 3 应用已经拆分。租户创建、机构类型和机构树、用户、角色、功能权限、数据范围、账号激活与恢复、密码/短信登录、手机号绑定/换绑/解绑、自助找回、设备会话、租户安全策略、中文审计、可靠 Outbox 与 Auth 幂等全设备下线均已进入真实代码。`AUTH_DDL_0010`、`IAM_DML_0006` 与 `IAM_DML_0007` 已由用户确认在 LOCAL PostgreSQL 执行并冻结；平台租户资料修改、暂停、恢复和不可恢复注销闭环及其权限资源已经落地。
