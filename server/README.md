...
当前登录租户账号可以通过 `/api/auth/v1/mobile/binding` 查询脱敏绑定状态，通过受登录保护的挑战和确认接口完成首次绑定。确认时 Auth 会再次核对提交手机号与挑战摘要，随后使用 AES-256-GCM 随机 IV 加密完整手机号，并只在界面和审计中返回脱敏号码。同一账号只能存在一个有效绑定，同一手机号摘要允许关联多个不同账号。

手机号换绑和解绑使用独立敏感流程，禁止复用首次绑定接口覆盖原记录。账号必须先通过当前手机号验证码或当前密码取得五分钟单用途再认证票据；换绑时还必须验证新手机号，且创建新手机号挑战本身也要求换绑票据。成功后 Auth 在一个本地事务内替代或撤销旧绑定、消费再认证票据并失效数据库设备会话，随后使该账号全部 Sa-Token 会话立即失效；审计由 `@AuditOperation` 在业务提交后使用统一有界线程池异步追加，不再与手机号事务强耦合。当前手机号与密码均不可用时，先走已有管理员密码恢复流程，恢复登录后仍须完成新手机号验证，项目不伪造尚未发布的管理员直接改绑权限。
...
操作人禁止由前端或业务入参提交。切面在原业务线程调用方法前，通过当前服务的 `AuditActorResolver` 从 Sa-Token 登录 ID 和 Token Session 捕获租户 ID、机构 ID、账号或平台身份 ID、用户名、显示名称及应用端编码；这份不可变快照随事件进入异步线程。异步消费者不得再次读取 Sa-Token 上下文。无登录内部任务必须明确记录为 `SYSTEM`，不得冒用普通用户。
